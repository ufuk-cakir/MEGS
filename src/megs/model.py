...
class DataLoader():
    ...
    def get_image(self, particle_type, field, index=None, ignore_mask = False):
        ...
        with h5py.File(self.path, 'r') as f:
            if index is None:
                # Check if mask is defined
                if hasattr(self, 'mask') and not ignore_mask:
                    return f[f"Galaxies/Particles/{particle_type}/Images/{field}"][self.mask]
                else:
...
class mPCA():
    ...
    def __init__(self,data, particle_type = None, norm_function = None, norm_function_kwargs=None, mask = None):
